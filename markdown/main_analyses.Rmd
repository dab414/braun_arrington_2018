...
2. The weighing of effort against reward.
    ...
    (b) Assessing whether the cost of effort increases over time.
        * If true, we expect that partitipants will require more reward for the same amount of effort as the experiment progresses.
...
Checking model fit is a notoriously difficult thing to do for mixed logistic models... 
...
There is a lack of consensus as far as the assumptions for logistic regressions with random effects. If I see evidence that residuals for both fixed and random effects are normally distributed, and that residuals are homogeneously distributed around the predicted values, I'll be happy with the model fit. 
...
Here the main effect of block doesn't look significant at all. But again, the model failed to converge. Running a step-down model:
